...
**Активний фокус:** RAG-трек: retrieval → query → generation (`POST /answer`) → eval. Eval-каркас
`eval_common` + generation-eval v1 (merged). Після першого прогону виявлено **конфаунд**: live-retrieval
у generation-eval змішує retrieval з генерацією → **scope-ревізія**: generation-eval звужено до
**ізольованої генерації на gold-контексті** (faithfulness+completeness); refusal+поріг+end-to-end RAG
**запарковано** окремим треком. Далі — ревізія generation-eval **v2** + κ-калібрування судді. Park:
RAG-eval+threshold, recheck-луп verifier, AWS deploy.
