...
**Навіщо ОБИДВІ (recall не субсумує answer-rate):** кожен answerable-кейс має 3 результати, а recall злипає два з них — `recall=0` означає **або** відмову (немає джерела ≤T), **або** відповідь із ЧУЖОГО джерела (очікуване не знайшлось, але інше ≤T). recall їх не розрізняє; answer-rate — так. Розрив **`answer-rate − recall` = частка відповідей із хибного джерела** — найгірша тиха помилка фактчекера (впевнена відповідь на нерелевантному контексті), гірша за відмову, і recall її сам **не видно**. Поріг її не лагодить (це precision retrieval — чуже джерело надто близько до запиту), але звіт її показує → сигнал, чи довіряти точці, чи чинити retrieval. У прикладі нижче a002@T=0.40 — саме цей кейс (`answer-rate=1, recall=0`: відповідь із чужого `p7`).
