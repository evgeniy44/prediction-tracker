...
**Дата:** 2026-04-26
**Status:** ✅ implemented (`src/prophet_checker/sources/telegram.py:TelegramSource.collect()`)
...
flowchart TD
    ...
    Source["src/prophet_checker/sources/telegram.py<br/>TelegramSource.collect()"]:::script
    ...
    TG -- "iter_messages()" --> Source
    Source --> Filter --> Sample --> JSON
...
## Task 21 — ✅ done 2026-04-29

`collect_channel` переселено в `src/prophet_checker/sources/telegram.py:TelegramSource` з єдиним `Source.collect()` інтерфейсом. Output писатиметься через `IngestionOrchestrator` (Task 15, planned) в Postgres замість JSON-файлу. Скрипт `scripts/collect_telegram_posts.py` видалено.
