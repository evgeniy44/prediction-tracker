...
R4. MODALITY PRESERVATION — the most common extraction error. Reconstruction
    ...
    - NEVER strengthen a hedge during reconstruction: "сприятиме X" ≠ "призведе
      до X"; "має шанси" ≠ "станеться".
    - BUT a hedged forecast WITH a concrete checkable outcome or timeframe IS
      a valid prediction — extract it and PRESERVE the hedge in claim_text
      ("ймовірно", "може", "очікує"). DROP only when even the hedged content
      has no checkable criterion.
...
EXAMPLE (hedged but concrete — extract WITH the hedge preserved):
Source: "Всего за год будет где-то 700 миллионов долларов военной помощи."
WRONG -> (nothing extracted — dropped just because "где-то" is a hedge)
RIGHT -> "Сукупна військова допомога США Україні за рік становитиме близько
         700 мільйонів доларів." (hedge preserved; amount and timeframe are checkable)
...
A hedged claim with a concrete outcome or timeframe ("ймовірно X до квітня",
"близько 700 млн за рік") PASSES this gate — preserve the hedge, do not drop.
